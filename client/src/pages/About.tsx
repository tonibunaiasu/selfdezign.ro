...
export default function About() {
  ...
  const aboutLayout = page?.aboutLayout;
  const layoutReady = Boolean(
    aboutLayout?.heroTitle ||
      aboutLayout?.positioningLeftTitle ||
      aboutLayout?.quoteLead
  );
  ...
  return (
    <div className="min-h-screen bg-background">
      <SEO
        title={page?.seoTitle || t.about.title}
        description={page?.seoDescription || t.about.subtitle}
        url="/despre"
      />
      {layoutReady ? (
        <>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
                <div className="space-y-8">
                  <h2 className="text-3xl md:text-4xl font-display font-bold tracking-tighter">
                    {aboutLayout?.positioningLeftTitle || t.about.notAbout1}
                  </h2>
                  <div className="space-y-6 text-lg text-gray-600 leading-relaxed">
                    {(splitLines(aboutLayout?.positioningLeftBody).length
                      ? splitLines(aboutLayout?.positioningLeftBody)
                      : [t.about.notAbout2, t.about.notAbout3]
                    ).map((line, index) => (
                ...
                <div className="space-y-8">
                  <h2 className="text-3xl md:text-4xl font-display font-bold tracking-tighter">
                    {aboutLayout?.positioningRightTitle || t.about.isAbout1}
                  </h2>
                  <div className="space-y-6 text-lg text-gray-600 leading-relaxed">
                    {(splitLines(aboutLayout?.positioningRightBody).length
                      ? splitLines(aboutLayout?.positioningRightBody)
                      : [t.about.isAbout2, t.about.isAbout3, t.about.isAbout4]
                    ).map((line, index) => (
                      <p
                        key={`right-${index}`}
                        className={
                          index === 2 && !aboutLayout?.positioningRightBody
            ...
            <div className="container">
              <div className="max-w-4xl mx-auto text-center">
                <blockquote className="text-2xl md:text-4xl font-display font-bold tracking-tight leading-tight mb-8">
                  „{aboutLayout?.quoteLead || t.about.conclusion1}
                  <span className="text-accent">
                    {" "}
                    {aboutLayout?.quoteHighlight || t.about.conclusion2}
                  </span>
                  "
                </blockquote>
            ...
            <div className="container">
              <div className="text-center mb-16">
                <h2 className="text-3xl md:text-4xl font-display font-bold tracking-tighter mb-4">
                  {aboutLayout?.signatureTitle || t.about.signature}
                </h2>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto">
                <Link href={aboutLayout?.navOneHref || "/viziune"}>
                  <a className="group block bg-white p-8 border border-gray-200 hover:border-accent transition-colors">
                    ...
                    <h3 className="text-2xl font-display font-bold mb-2 group-hover:text-accent transition-colors">
                      {aboutLayout?.navOneLabel || t.about.visionLink}
                    </h3>
                  </a>
                </Link>

                <Link href={aboutLayout?.navTwoHref || "/valori"}>
                  <a className="group block bg-white p-8 border border-gray-200 hover:border-accent transition-colors">
                    <div className="flex items-start justify-between mb-4">
                      <span className="text-accent text-sm font-bold uppercase tracking-widest">
                        02
                    ...
                    <h3 className="text-2xl font-display font-bold mb-2 group-hover:text-accent transition-colors">
                      {aboutLayout?.navTwoLabel || t.about.valuesLink}
                    </h3>
                  </a>
                </Link>
              </div>
          ...
          <section className="py-24 bg-white">
            <div className="container text-center">
              <Link href={aboutLayout?.ctaHref || "/contact"}>
                <Button className="bg-[var(--color-brand-yellow)] text-black hover:bg-[var(--color-brand-yellow)]/90 rounded-none uppercase tracking-widest font-bold px-8 py-6 text-sm">
                  {aboutLayout?.ctaLabel || t.nav.contact}
                </Button>
              </Link>
            </div>
          </section>
